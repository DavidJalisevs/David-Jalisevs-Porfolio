...
void JumpPlayerState::update(Player& player) {
	...
	player.getAnimatedSprite().move(0, -0.4);
	...
	DEBUG_MSG("JumpPlayerState -> DoubleJumpState");
	if (m_clock.getElapsedTime().asSeconds() < 1.3f && sf::Keyboard::isKeyPressed(sf::Keyboard::J))
	{
		PlayerState* temp = player.getPlayerState();
		PlayerState* state = new DoubleJumpState();
		player.getPlayerState()->exit(player);
		player.setPlayerState(state);
		player.getPlayerState()->enter(player);
		delete temp;
